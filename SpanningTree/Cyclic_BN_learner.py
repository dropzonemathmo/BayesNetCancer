...
def SpanningTreeAlgorithm(depList, noVariables):
	...
	if checkNotIn(v1,visitedNodes):
		visitedNodes += [v1]
	if checkNotIn(v2,visitedNodes):
		visitedNodes += [v2]
	cyclicSpan += [[w, v1, v2]]
	if len(visitedNodes) == noVariables:
		break
...
outputFile = textFilePath+str(quant_no)+'-'+str(unique)+'cycle.csv'

writeTree(textFilePath+'.csv',quant_no,unique,outputFile)

outputGraph = textFilePath+str(quant_no)+'-'+str(unique)+'cycle.png'
